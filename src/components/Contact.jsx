  ...
  return (
    <section id="contact" ref={ref} style={{ padding:secPad,background:"rgba(255,255,255,0.013)",borderTop:"1px solid rgba(255,255,255,0.05)" }}>
      <div style={{ maxWidth:900,margin:"0 auto",textAlign:"center" }}>
        <p style={{ fontFamily:"'Space Mono',monospace",fontSize:10,color:"#D4A853",letterSpacing:"0.18em",textTransform:"uppercase",marginBottom:20,opacity:inView?1:0,transition:"opacity 0.6s ease" }}>04 — Contact</p>
        <div style={{ marginBottom: isMobile ? 36 : 52 }}>
          {[["LET'S BUILD",0.1,"#fff"],["SOMETHING",0.35,"transparent"],["GREAT.",0.62,"#fff"]].map(([t,d,c],i)=>(
            <div key={i} style={{ overflow:"hidden",display:"block" }}>
              <SplitText text={t} inView={inView} delay={d} style={{ fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:"clamp(36px,11vw,132px)",color:c,WebkitTextStroke:c==="transparent"?"1.5px rgba(212,168,83,0.65)":undefined,lineHeight:0.88,letterSpacing:"0.02em" }}/>
            </div>
          ))}
        </div>
        <div style={{ opacity:inView?1:0,transform:inView?"none":"translateY(24px)",transition:"all 0.8s ease 0.85s",marginBottom: isMobile ? 44 : 64,display:"flex",justifyContent:"center" }}>
          <button data-h="true" onClick={copy} style={{
            ...
            border:`1px solid ${copied?"rgba(212,168,83,0.4)":"rgba(255,255,255,0.1)"}`,
            background:copied?"rgba(212,168,83,0.06)":"transparent",
            cursor: isMobile ? "pointer" : "none",fontFamily:"'Space Mono',monospace",
            fontSize: isMobile ? 12 : 15,color:copied?"#D4A853":"#fff",letterSpacing:"0.06em",
            transition:"all 0.4s",maxWidth:"100%",
          }}
            onMouseEnter={e=>{if(!copied){e.currentTarget.style.borderColor="#D4A853";e.currentTarget.style.boxShadow="0 0 48px rgba(212,168,83,0.1)";}}}
      ...
      <div style={{ maxWidth:1200,margin: isMobile ? "48px auto 0" : "80px auto 0",paddingTop:32,borderTop:"1px solid rgba(255,255,255,0.05)",display:"flex",flexDirection: isMobile ? "column" : "row",gap: isMobile ? 16 : 0,justifyContent:"space-between",alignItems:"center",position:"relative" }}>
        <span style={{ fontFamily:"'Space Mono',monospace",fontSize: isMobile ? 9 : 10,color:"rgba(255,255,255,0.16)",letterSpacing:"0.08em" }}>© 2026</span>
      </div>
    </section>
  );
}
