...
export default function About() {
  ...
  return (
    <section id="about" ref={ref} style={{ padding:"130px 52px",maxWidth:1200,margin:"0 auto" }}>
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:80,alignItems:"center" }}>
        <div>
          ...
          {[`Frontend developer with ${getYearsExp()} years of experience specializing in React.js, real-time interfaces, and responsive UI development. Proven track record delivering enterprise-grade web apps and chatbot platforms.`,
            ...
            <p key={i} style={{ fontFamily:"'DM Sans',sans-serif",fontSize:16,lineHeight:1.85,color:"rgba(255,255,255,0.48)",marginBottom:16,opacity:inView?1:0,transform:inView?"none":"translateX(-24px)",transition:`all 0.85s ease ${0.42+i*0.15}s` }}>{p}</p>
          ))}
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:3 }}>
          {stats.map((s,i)=>(
            <TiltCard key={s.label} accent={s.color} style={{ opacity:inView?1:0,transform:inView?"none":"translateY(36px) scale(0.96)",transition:`all 0.85s cubic-bezier(0.16,1,0.3,1) ${0.15+i*0.1}s` }}>
              <div style={{ padding:"40px 28px",border:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.018)" }}>
                <div style={{ fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:66,color:s.color,lineHeight:1,textShadow:`0 0 44px ${s.color}55` }}>{s.value}</div>
                <div style={{ fontFamily:"'Space Mono',monospace",fontSize:10,color:"rgba(255,255,255,0.28)",letterSpacing:"0.1em",textTransform:"uppercase",marginTop:8 }}>{s.label}</div>
              </div>
            </TiltCard>
          ))}
        </div>
