  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "'Bebas Neue',Impact,sans-serif", fontSize: 72, color: "#fff", letterSpacing: "0.06em", animation: "flicker 2.5s ease infinite" }}>
          SP<span style={{ color: "#00FFB2" }}>.</span>
      </div>
      <div style={{ width: 220, height: 1, background: "rgba(255,255,255,0.07)" }}>
        <div style={{ height: "100%", width: `${pct}%`, background: "linear-gradient(90deg,#00FFB2,#00D4FF)", boxShadow: "0 0 12px #00FFB2", transition: "width 0.08s" }} />
      </div>
      ...
      {[[20,20,"tl"],[20,null,null],[null,20,"bl"],[null,null,"br"]].map(([t,l,p],i) => (
        <div key={i} style={{
          ...
          borderTop:["tl","tr"].includes(p)?"1px solid rgba(0,255,178,0.3)":undefined,
          borderBottom:["bl","br"].includes(p)?"1px solid rgba(0,255,178,0.3)":undefined,
          borderLeft:["tl","bl"].includes(p)?"1px solid rgba(0,255,178,0.3)":undefined,
          borderRight:["tr","br"].includes(p)?"1px solid rgba(0,255,178,0.3)":undefined,
        }}/>
      ))}
    </div>
  );
