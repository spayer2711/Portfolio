...
const SKILLS = [
  ...
  { name:"Node.js / Express.js", level:78, color:"#FF6B35" },
...
export default function Skills() {
  ...
  return (
    <section id="skills" ref={ref} style={{ padding:"130px 52px",background:"rgba(255,255,255,0.013)",borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
      <div style={{ maxWidth:1200,margin:"0 auto" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px 72px",marginBottom:72 }}>
          {SKILLS.map((sk,i)=>(
            <div key={sk.name} data-h="true"
              onMouseEnter={()=>setHovered(sk.name)} onMouseLeave={()=>setHovered(null)}
              style={{ opacity:inView?1:0,transform:inView?"none":"translateX(-28px)",transition:`all 0.85s cubic-bezier(0.16,1,0.3,1) ${i*0.07}s`,cursor:"none" }}
            >
              <div style={{ display:"flex",justifyContent:"space-between",marginBottom:9 }}>
                <span style={{ fontFamily:"'DM Sans',sans-serif",fontSize:14,color:hovered===sk.name?sk.color:"rgba(255,255,255,0.62)",transition:"color 0.25s" }}>{sk.name}</span>
        ...
        <div style={{ display:"flex",flexWrap:"wrap",gap:10,opacity:inView?1:0,transition:"opacity 1.2s ease 1.1s" }}>
          {TAGS.map((tag,i)=>(
